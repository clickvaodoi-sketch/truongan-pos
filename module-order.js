    ...
    try {
        ...
        let filtered = ALL_ORDERS.filter(o => {
            ...
            if(s) {
                ...
                let phone = cleanPhone(o['SDT']); 
                let orderId = String(o['Mã Đơn'] || '').toLowerCase();
        ...
        filtered.sort((a, b) => {
            let dateA = parseDateString(a['Thời Gian']).getTime();
            let dateB = parseDateString(b['Thời Gian']).getTime();
        ...
        let essentialCols = ['Mã Đơn', 'Tên Khách Hàng', 'SDT', 'Tổng Tiền'];
        ...
        sliced.forEach(o => {
          try {
              ...
              let btnText = status === 'Đã hoàn trả' ? '↩️' : (isDelivered ? '✔️' : (isDraft ? '💾' : '🚚'));
              ...
              if(viewMode === 'table') {
                 ...
                 vis.forEach(k => {
                    if(k !== 'Chi Tiết JSON') {
                       ...
                       if(k === 'Mã Đơn') { val = `<span onclick="openOrderModal('${maDon}')" style="color:#3b82f6; font-weight:bold; cursor:pointer; text-decoration:underline;" title="Bấm để xem chi tiết">${o['Mã Đơn']}</span>`; }
                       else if(k === 'Tên Khách Hàng') { val = `<span onclick="openOrderModal('${maDon}')" style="color:#3b82f6; font-weight:bold; cursor:pointer;" title="Bấm để xem chi tiết">${o[k]||''}</span> ${vipBadge}`; }
                       else if(k === 'Chiết Khấu %') {
                           let valNum = Number(o[k] || 0);
                           if (valNum > 0) {
                               let raw = Number(String(o['Tổng Tiền']||0).replace(/[^0-9\-]/g,""));
                               let finalT = Number(String(o['Thành Tiền Sau CK']||0).replace(/[^0-9\-]/g,""));
                 ...
                 vis.forEach(k => {
                    if(!['Mã Đơn', 'Trạng Thái', 'Chi Tiết JSON'].includes(k)) {
                       let isMoney = (String(k).toLowerCase().includes('tiền') || String(k).toLowerCase().includes('giá') || String(k).toLowerCase().includes('thanh toán') || String(k).toLowerCase().includes('nợ'));
                       let val = isMoney ? `<span class="price-text">${formatMoney(o[k])}</span>` : (o[k] || '');
                       let titleVal = String(o[k] || '').replace(/"/g, '&quot;');
                       if(k === 'Chiết Khấu %') {
                           let valNum = Number(o[k] || 0);
                           if (valNum > 0) {
                               let raw = Number(String(o['Tổng Tiền']||0).replace(/[^0-9\-]/g,""));
                       ...
                       if(k === 'Tên Khách Hàng') { val = `<span onclick="openOrderModal('${maDon}')" style="color:#3b82f6; font-weight:bold; cursor:pointer;" title="Bấm để xem chi tiết">${o[k]||''}</span> ${vipBadge}`; }
...
function deleteOrder(id) {
        ...
        try {
           JSON.parse(String(o['Chi Tiết JSON'])).forEach(item => {
               ...
               if(p) {
                   ...
                   p[keyDangDat] = (Number(p[keyDangDat]) || 0) - Number(item.soLuong); if(p[keyDangDat] < 0) p[keyDangDat] = 0;
                   addQueueItem('updateProduct', p);
               }
           });
           localStorage.setItem('ALL_PRODUCTS', JSON.stringify(ALL_PRODUCTS));
    ...
    let debt = Number(o['Còn Nợ'] || 0);
    let cPhoneClean = cleanPhone(o['SDT']);
    if(debt > 0 && cPhoneClean) {
       let cus = ALL_CUSTOMERS.find(c => cleanPhone(c['Điện thoại']) === cPhoneClean);
       if(cus) { cus['Công nợ'] = (Number(cus['Công nợ']) || 0) - debt; addQueueItem('updateCustomer', cus); localStorage.setItem('ALL_CUSTOMERS', JSON.stringify(ALL_CUSTOMERS)); }
    }
...
function printOrder(id) {
    let o = ALL_ORDERS.find(x => x['Mã Đơn'] === id); if(!o) return;
    let finalTotal = Number(String(o['Thành Tiền Sau CK']||o['Tổng Tiền']||0).replace(/[^0-9\-]/g,"")); 
    let chietKhauPct = Number(o['Chiết Khấu %'] || 0);
    let tongTienHang = Number(String(o['Tổng Tiền']||0).replace(/[^0-9\-]/g,"")); let tienGiam = tongTienHang - finalTotal;
    let paid = Number(String(o['Khách Thanh Toán']||0).replace(/[^0-9\-]/g,""));
    ...
    let printContent = `<div style="font-family: monospace; width: 100%; max-width: 300px; margin: 0 auto; color: #000;">
        <h2 style="text-align: center; margin-bottom: 5px;">TRƯỜNG AN STORE</h2>
        <div style="text-align: center; font-size: 12px; border-bottom: 1px dashed #000; padding-bottom: 10px; margin-bottom: 10px;">HÓA ĐƠN BÁN HÀNG</div>
    ...
    if(o['Chi Tiết JSON']) {
       try { JSON.parse(String(o['Chi Tiết JSON'])).forEach(i => { printContent += `<div style="display:flex; justify-content:space-between; font-size:13px; margin-top:8px; border-bottom:1px dotted #eee; padding-bottom:5px;"><div style="flex:1; padding-right:5px;"><div>[${i.maSP}] ${i.tenSP}</div><div style="font-size:11px; color:#555;">${formatMoney(i.giaBan)} x ${i.soLuong}</div></div><div style="font-weight:bold; align-self:flex-end;">${formatMoney(i.thanhTien)}</div></div>`; }); } catch(e) {}
    }
...
function openOrderModal(id) { 
    ...
    let html = `<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;">
        <div>
            <h3 style="margin:0; color:#3b82f6;">${o['Mã Đơn']}</h3>
            <span style="font-size:12px; opacity:0.7;">${o['Thời Gian']}</span>
        </div>
        <div style="background:${statusColor}; color:#fff; padding:4px 10px; border-radius:12px; font-size:12px; font-weight:bold;">${o['Trạng Thái']}</div>
    </div>`;

    html += `<div class="order-detail-box">
        <div style="margin-bottom:5px;">👤 <b>Khách hàng:</b> ${o['Tên Khách Hàng']} ${isVIP?'<span class="vip-badge">🌟 VIP</span>':''}</div>
        <div style="margin-bottom:5px;">📞 <b>SĐT:</b> ${o['SDT']||'---'}</div>
        <div>📍 <b>Địa chỉ:</b> ${o['Địa Chỉ']||'---'}</div>
    </div>`;
    ...
    if(o['Chi Tiết JSON']) { 
       try { 
           JSON.parse(String(o['Chi Tiết JSON'])).forEach(i => { 
               rawTotal += i.thanhTien; 
               html += `<div class="order-item-row">
                   <div style="flex:1;"><b>[${i.maSP}] ${i.tenSP}</b> <br/><span style="font-size:12px; color:#64748b;">${formatMoney(i.giaBan)} x ${i.soLuong}</span></div>
                   <div style="font-weight:bold; color:#0070f4; display:flex; align-items:center;">${formatMoney(i.thanhTien)}</div>
               </div>`; 
           }); 
       } catch(e) {} 
    }
    
    let chietKhau = Number(o['Chiết Khấu %']||0); 
    let finalTotal = Number(String(o['Thành Tiền Sau CK']||o['Tổng Tiền']||0).replace(/[^0-9\-]/g,"")); 
    
    html += `</div><div class="order-detail-box">`;
    html += `<div style="display:flex; justify-content:space-between; font-size:13px; margin-bottom:8px;"><span>Tổng tiền hàng:</span> <b>${formatMoney(rawTotal)}</b></div>`;
    
    if(chietKhau > 0) { 
        let tienGiam = Math.abs(rawTotal) - Math.abs(finalTotal); 
        html += `<div style="display:flex; justify-content:space-between; font-size:13px; margin-bottom:8px; color:#10b981;"><span>Được giảm giá (${chietKhau}%):</span> <b>-${formatMoney(tienGiam)}</b></div>`; 
    }
    
    html += `<div style="display:flex; justify-content:space-between; align-items:center; border-top:1px dashed #cbd5e1; padding-top:10px; margin-top:5px;">
        <span style="font-size:15px; font-weight:bold;">THÀNH TIỀN:</span> 
        <span style="font-size:18px; font-weight:bold; color:#d32f2f;">${formatMoney(finalTotal)}</span>
    </div>`;

    let paid = Number(String(o['Khách Thanh Toán']||0).replace(/[^0-9\-]/g,"")); 
    let debt = Number(String(o['Còn Nợ']||0).replace(/[^0-9\-]/g,""));
    
    html += `<div style="display:flex; justify-content:space-between; background:var(--btn-bg, #f0fdf4); border:1px solid #bbf7d0; padding:10px; border-radius:6px; margin-top:10px;">
        <div style="text-align:center;"><span style="font-size:12px; opacity:0.7;">Đã thanh toán</span><br/><b style="color:#10b981;">${formatMoney(paid)}</b></div>
        <div style="text-align:center;"><span style="font-size:12px; opacity:0.7;">Còn nợ</span><br/><b style="color:#ef4444;">${formatMoney(debt)}</b></div>
    </div></div>`;

    html += `<div style="display:flex; gap:10px; margin-top:15px;">
        <button class="action-btn orange" style="flex:1; padding:12px; font-size:14px;" onclick="closeModal('orderModal'); openEditOrderModal('${id}')">✏️ SỬA ĐƠN NÀY</button>
        <button class="action-btn gray" style="flex:1; padding:12px; font-size:14px;" onclick="printOrder('${id}')">🖨️ IN HÓA ĐƠN</button>
    </div>`;
...
function openEditOrderModal(id) {
    ...
    if(cPhoneClean) {
        let cus = ALL_CUSTOMERS.find(c => cleanPhone(c['Điện thoại']) === cPhoneClean);
        if(cus) { 
            cOldDebt = getRealtimeCustomerDebt(cPhoneClean) - Number(String(o['Còn Nợ']||0).replace(/[^0-9\-]/g,"")); 
            let ect = document.getElementById('eoCustomerType'); if(ect) ect.value = cus['Nhóm KH'] || 'Khách Lẻ';
        }
    }
...
function saveEditOrder() {
    ...
    o['Thành Tiền Sau CK'] = finalTotal; o['Khách Thanh Toán'] = paid; o['Còn Nợ'] = newDebt;
    ...
    if (cPhoneClean) {
       ...
       if(cusIdx === -1) { 
           let cus = { "Mã khách hàng": generateCustomerId(), "Tên khách hàng": nEl.value, "Điện thoại": cPhoneClean, "Địa Chỉ": aEl.value, "Nhóm KH": cusType, "Công nợ": 0, "Tổng đơn hàng": 0, "Tổng mua": 0 };
...
function getProductPriceInfo(p, qty = 1, cusType = 'Khách Lẻ') {
    ...
    if (isSỉ || (giaSi > 0 && qty >= siTu)) {
        let finalSi = giaSi > 0 ? giaSi : basePrice;
        return { price: finalSi, isWholesale: true, basePrice: finalSi, appliedKm: 0 };
    }
...
function changePriceType(isEditMode) {
    ...
    currentOrderItems.forEach(item => {
        let p = ALL_PRODUCTS.find(x => x['Mã SP'] == item.maSP);
        if(p) {
            let priceInfo = getProductPriceInfo(p, item.soLuong, pType);
            item.giaBan = priceInfo.price; item.ckPercent = priceInfo.appliedKm || ''; item.thanhTien = item.soLuong * item.giaBan;
        }
    });
...
function saveDraftLocal() {
    ...
    let no = document.getElementById('cNote'); let d = document.getElementById('cDiscount'); let pd = document.getElementById('cPaid');
...
function restoreDraftLocal() {
    ...
    if(draftStr) {
        try {
            ...
            let n = document.getElementById('cName'); if(n) n.value = draft.name || '';
            let p = document.getElementById('cPhone'); if(p) p.value = draft.phone || '';
            let a = document.getElementById('cAddress'); if(a) a.value = draft.address || '';
            let no = document.getElementById('cNote'); if(no) no.value = draft.note || '';
            let d = document.getElementById('cDiscount'); if(d) d.value = draft.discount || '';
            let pd = document.getElementById('cPaid'); if(pd) pd.value = draft.paid || '';
...
function clearDraftLocal() {
    ...
    let ids = ['cName', 'cPhone', 'cAddress', 'cNote', 'cDiscount', 'cPaid'];
...
function buildCustomerList() {
    ...
    ALL_ORDERS.forEach(o => {
        ...
        if(sdt && !list.find(c => cleanPhone(c['Điện thoại']) === sdt)) {
            list.push({ 'Mã khách hàng': '', 'Tên khách hàng': name, 'Điện thoại': sdt, 'Địa Chỉ': String(o['Địa Chỉ']||''), 'Nhóm KH': 'Khách Lẻ', 'Công nợ': 0, 'Tổng đơn hàng': 0, 'Tổng mua': 0 });
        }
    });
    return list;
}
...
function showCustomerSuggest(inputEl, isEditMode, boxIdOverride) {
    ...
    let list = buildCustomerList().filter(c => {
        let matchPhone = valClean ? cleanPhone(c['Điện thoại']).includes(valClean) : false;
        let matchName = String(c['Tên khách hàng']||'').toLowerCase().includes(valRaw);
    ...
    let html = list.map(c => {
        ...
        let cType = c['Nhóm KH'] || 'Khách Lẻ'; let noStr = (stats.currentDebt > 0) ? `<span class="debt-badge">Nợ cũ: ${formatMoney(stats.currentDebt)}</span>` : '';
        return `<div class="suggest-item" onmousedown="selectCustomer('${String(c['Tên khách hàng']||'').replace(/'/g,"\\'")}', '${phone}', '${String(c['Địa Chỉ']||'').replace(/'/g,"\\'")}', ${stats.currentDebt}, '${cType}', ${isEditMode}, '${boxId}')">
...
function selectCustomer(name, phone, address, oldDebt, cusType, isEditMode, boxId) {
    if(isEditMode) {
        let en = document.getElementById('eoName'); if(en) en.value = name;
        let ep = document.getElementById('eoPhone'); if(ep) ep.value = phone;
        let ea = document.getElementById('eoAddress'); if(ea) ea.value = address;
        let eod = document.getElementById('eoOldDebt'); if(eod) eod.value = oldDebt;
        let ctSel = document.getElementById('eoCustomerType'); if(ctSel) { ctSel.value = cusType; changePriceType(true); }
        updateOrderSummary(true);
    } else {
        let cn = document.getElementById('cName'); if(cn) cn.value = name;
        let cp = document.getElementById('cPhone'); if(cp) cp.value = phone;
        let ca = document.getElementById('cAddress'); if(ca) ca.value = address;
        let cod = document.getElementById('cOldDebt'); if(cod) cod.value = oldDebt;
        let ctSel = document.getElementById('cCustomerType'); if(ctSel) { ctSel.value = cusType; changePriceType(false); }
        updateOrderSummary(false); saveDraftLocal();
    }
    if(typeof closeSuggest === 'function') closeSuggest(boxId);
}
...
function updateItem(idx, field, val, isEditMode) {
    ...
    if(field === 'qty') { 
        ...
        if(p) {
            ...
            let pTypeEl = document.getElementById(isEditMode ? "eoCustomerType" : "cCustomerType");
            let pType = pTypeEl ? pTypeEl.value : 'Khách Lẻ';
            let priceInfo = getProductPriceInfo(p, val, pType);
            item.giaBan = priceInfo.price; item.ckPercent = priceInfo.appliedKm || '';
        }
...
function updateOrderSummary(isEditMode) {
    let discId = isEditMode ? "eoDiscount" : "cDiscount"; 
    let sumId = isEditMode ? "eoSummary" : "addOrderSummary";
    let paidId = isEditMode ? "eoPaid" : "cPaid";
    let oldDebtId = isEditMode ? "eoOldDebt" : "cOldDebt";
    ...
    let html = `<div class="summary-row"><span>Tiền hàng đơn này:</span> <span>${formatMoney(rawTotal)}</span></div>`;
    if(discVal > 0) html += `<div class="summary-row discount"><span>Giảm giá hóa đơn (${disc}%):</span> <span>-${formatMoney(discVal)}</span></div>`;
...
function autoFillPaid(type) {
    ...
    let dEl = document.getElementById("cDiscount"); let disc = dEl ? (Number(dEl.value) || 0) : 0;
    let rawTotal = currentOrderItems.reduce((sum, item) => sum + item.thanhTien, 0);
    let finalTotal = disc > 0 ? smartRound(rawTotal * (1 - disc/100)) : rawTotal;
    let amountToFill = 0;
    if (type === 'current') { amountToFill = finalTotal; } else if (type === 'all') { let odEl = document.getElementById("cOldDebt"); let oldDebt = odEl ? (Number(odEl.value) || 0) : 0; amountToFill = finalTotal + oldDebt; }
    let paidInput = document.getElementById("cPaid");
    if (paidInput) { paidInput.value = amountToFill; updateOrderSummary(false); saveDraftLocal(); }
}
...
function renderAddFormUI() { 
    ...
    let opts = ALL_PRODUCTS.map(p => {
        ...
        let textStr = `${p['Tên SP']} | Giá: ${formatMoney(priceInfo.price)} | Tồn: ${validStock} | Đang đặt: ${Number(p[getKeyByKeyword(p, 'đang đặt')])||0}`;
        if (validStock <= 0) return `<option value="${p['Mã SP']}" disabled>❌ HẾT HÀNG - ${textStr}</option>`;
        return `<option value="${p['Mã SP']}">${textStr}</option>`;
    }).join('');
    let dl = document.getElementById("productDatalist"); if(dl) dl.innerHTML = opts; 
    renderOrderItemsList(false);
}

function addTempItemToOrder(isEditMode) {
    let inputId = isEditMode ? "productInputEdit" : "productInputAdd";
    let prodEl = document.getElementById(inputId); let valId = prodEl ? String(prodEl.value).trim().toUpperCase() : ''; 
    ...
    let validStock = (Number(p[getKeyByKeyword(p, 'tồn kho')])||0) - (Number(p[getKeyByKeyword(p, 'đang đặt')])||0);
    let existing = currentOrderItems.find(i => i.maSP == p['Mã SP']);
    let currentTempQty = existing ? existing.soLuong : 0;
    ...
    if(existing) { 
        ...
        currentOrderItems.push({ maSP: p['Mã SP'], tenSP: p['Tên SP'], soLuong: qty, giaGoc: priceInfo.basePrice, giaBan: priceInfo.price, ckPercent: priceInfo.appliedKm || '', thanhTien: priceInfo.price * qty }); 
    }
...
function handleAddOrder(orderStatus = 'Chờ xử lý') {
    ...
    let newOrder = {
        ...
        "Chiết Khấu %": discountPct, "Thành Tiền Sau CK": finalTotal, "Chi Tiết JSON": JSON.stringify(currentOrderItems),
    ...
    if(orderStatus !== 'Nháp') {
        currentOrderItems.forEach(item => {
            let p = ALL_PRODUCTS.find(x => x['Mã SP'] == item.maSP);
            if(p) { let keyDangDat = getKeyByKeyword(p, 'đang đặt') || 'Đang đặt'; p[keyDangDat] = (Number(p[keyDangDat]) || 0) + item.soLuong; addQueueItem('updateProduct', p); }
        });
        localStorage.setItem('ALL_PRODUCTS', JSON.stringify(ALL_PRODUCTS));
        ...
        if (cPhoneClean) {
            let cusIdx = ALL_CUSTOMERS.findIndex(c => cleanPhone(c['Điện thoại']) === cPhoneClean);
            if(cusIdx === -1) { 
                let cus = { "Mã khách hàng": generateCustomerId(), "Tên khách hàng": cName, "Điện thoại": cPhoneClean, "Địa Chỉ": aEl ? aEl.value : '', "Nhóm KH": cusType, "Công nợ": 0, "Tổng đơn hàng": 0, "Tổng mua": 0 };
                ALL_CUSTOMERS.push(cus); addQueueItem('updateCustomer', cus);
            } else {
                let cus = ALL_CUSTOMERS[cusIdx]; let changed = false;
    ...
    localStorage.removeItem('draftOrderData'); 
    if(nEl) nEl.value = ''; if(pEl) pEl.value = ''; if(aEl) aEl.value = ''; 
    if(noEl) noEl.value = ''; if(dEl) dEl.value = ''; if(paidEl) paidEl.value = ''; if(odEl) odEl.value = 0; 
